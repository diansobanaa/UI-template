...
import { ArrowLeft, Calendar, Clock, Leaf, RotateCcw, Save, Settings2, Sprout, Trash2, Wheat, Plus, Wrench, X } from "lucide-react";
...
export function CycleManageModal({gh,cycle,open,onClose,onUpdateTanggalTanam,onUpdateTanggalPolinasi,onUpdateMetadata,onDeleteTanggalPolinasi,onResetCycle}:CycleManageModalProps){
 ...
 {view==="tanam"&&<div className="space-y-4"><div><Label htmlFor="manage-tanam">Tanggal Tanam</Label><Input id="manage-tanam" type="date" max={today} value={tanamDate} onChange={e=>{setTanamDate(e.target.value);setError(null)}} className="mt-1.5 w-full"/></div><div className="grid grid-cols-2 gap-3"><div className="rounded-xl bg-slate-50 p-3"><div className="text-[11px] text-slate-500">HST saat ini</div><div className="mt-1 text-xl font-bold">{hst} hari</div></div><div className="rounded-xl bg-emerald-50 p-3"><div className="text-[11px] text-emerald-600">Setelah diubah</div><div className="mt-1 text-xl font-bold text-emerald-700">{nextHst} hari</div></div></div></div>}
 {view==="polinasi"&&<div className="space-y-4">{cycle?.tanggalPolinasi?<><div><Label htmlFor="manage-pol">Tanggal Polinasi</Label><Input id="manage-pol" type="date" min={cycle.tanggalTanam||undefined} max={today} value={polDate} onChange={e=>{setPolDate(e.target.value);setError(null)}} className="mt-1.5 w-full"/></div><div className="rounded-xl bg-violet-50 p-4"><div className="text-xs text-violet-600">HSP setelah perubahan</div><div className="mt-1 text-2xl font-bold text-violet-700">{nextHsp} hari</div></div><Button variant="danger" onClick={()=>setConfirmDelete(true)} disabled={busy}><Trash2 className="mr-1.5 h-4 w-4"/>Hapus Tanggal Polinasi</Button></>:<div className="rounded-2xl border border-dashed border-slate-300 p-8 text-center text-sm text-slate-500">Belum ada tanggal polinasi. Catat polinasi dari kartu siklus di dashboard.</div>}</div>}
 {view==="tanaman"&&<div className="space-y-4"><div><Label htmlFor="manage-variety">Varietas</Label><Input id="manage-variety" value={variety} onChange={e=>setVariety(e.target.value)} className="mt-1.5 w-full"/></div><div><Label htmlFor="manage-plants">Jumlah Tanaman</Label><Input id="manage-plants" type="number" min={1} value={plants} onChange={e=>setPlants(Number(e.target.value))} className="mt-1.5 w-full"/></div><div><Label htmlFor="manage-notes">Catatan</Label><textarea id="manage-notes" value={notes} onChange={e=>setNotes(e.target.value)} className="mt-1.5 min-h-20 w-full rounded-xl border border-slate-200 p-3 text-sm"/></div></div>}
 {view==="timeline"&&<div className="space-y-5">
 <div>
  <Label htmlFor="target-hst">Rencana Panen (HST)</Label>
  <Input id="target-hst" type="number" min={1} value={target} onChange={e=>setTarget(e.target.value)} className="mt-1.5 w-full"/>
 </div>
 <section className="space-y-3">
  <div className="flex items-center justify-between gap-3">
   <div><div className="text-sm font-bold text-slate-900">Fase Masa Tanam</div><div className="text-xs text-slate-500">Rentang HST yang tampil sebagai fase utama di dashboard.</div></div>
   <Button variant="secondary" onClick={()=>setPoints(a=>[...a,{id:`custom-${Date.now()}`,name:"",startHst:"",endHst:"",note:""}])}><Plus className="mr-1.5 h-4 w-4"/>Tambah Fase</Button>
  </div>
  <div className="space-y-2">{points.map(p=><div key={p.id} className="rounded-xl border border-slate-200 p-3"><div className="flex items-center gap-2"><Input value={p.name} onChange={e=>setPoints(a=>a.map(x=>x.id===p.id?{...x,name:e.target.value}:x))} placeholder="Nama fase"/><button type="button" className="text-xs text-red-500" onClick={()=>setPoints(a=>a.filter(x=>x.id!==p.id))}>Hapus</button></div><div className="mt-2 grid grid-cols-2 gap-2"><Input type="number" value={p.startHst} onChange={e=>setPoints(a=>a.map(x=>x.id===p.id?{...x,startHst:e.target.value}:x))} placeholder="Mulai HST"/><Input type="number" value={p.endHst} onChange={e=>setPoints(a=>a.map(x=>x.id===p.id?{...x,endHst:e.target.value}:x))} placeholder="Selesai HST"/></div><div className="mt-2"><Input value={p.note} onChange={e=>setPoints(a=>a.map(x=>x.id===p.id?{...x,note:e.target.value}:x))} placeholder="Catatan fase (opsional)"/></div></div>)}</div>
 </section>

 </div>}
 {view==="maintenance"&&<div className="space-y-4"><div className="rounded-2xl border border-amber-200 bg-amber-50 p-4"><div className="flex items-start gap-3"><span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-amber-100 text-amber-700"><Wrench className="h-5 w-5"/></span><div><div className="text-sm font-bold text-amber-950">Rencana Jadwal Perawatan</div><div className="mt-1 text-xs leading-5 text-amber-800">Rencanakan tindakan perawatan berdasarkan HST. Jadwal yang disimpan akan ditampilkan sebagai point pada Timeline Masa Tanam.</div></div></div></div><div className="rounded-2xl border border-slate-200 bg-white p-4"><div className="flex items-center justify-between gap-3"><div><div className="text-sm font-bold text-slate-900">Jadwal saat ini</div><div className="mt-1 text-xs text-slate-500">{maintenance.length===0?"Belum ada jadwal perawatan.":`${maintenance.length} jadwal tersimpan.`}</div></div><Button variant="secondary" onClick={()=>setMaintenanceModalOpen(true)}><Wrench className="mr-1.5 h-4 w-4"/>Kelola Jadwal</Button></div>{maintenance.length>0&&<div className="mt-4 space-y-2">{[...maintenance].sort((a,b)=>Number(a.hst)-Number(b.hst)).map(m=><div key={m.id} className="flex items-center justify-between rounded-xl border border-slate-200 px-3 py-2.5"><div className="min-w-0"><div className="truncate text-xs font-bold text-slate-800">{m.name}</div><div className="mt-0.5 truncate text-[11px] text-slate-500">{m.category}{m.note?` • ${m.note}`:""}</div></div><div className="ml-3 shrink-0 rounded-lg bg-amber-100 px-2 py-1 text-[11px] font-bold text-amber-700">HST {m.hst}</div></div>)}</div>}</div></div>}
 {view==="opsi"&&<div className="space-y-3"><div className="rounded-2xl border border-amber-200 bg-amber-50 p-4"><div className="flex gap-3"><RotateCcw className="h-5 w-5 text-amber-600"/><div><div className="text-sm font-bold text-amber-900">Reset Siklus</div><div className="mt-1 text-xs leading-5 text-amber-800">Kembalikan status siklus dan gunakan alur mulai tanam dari awal.</div><Button variant="danger" className="mt-3" onClick={()=>setConfirmReset(true)} disabled={!onResetCycle||busy}>Reset Siklus</Button></div></div></div></div>}
 {error&&<div className="mt-4 rounded-xl border border-red-200 bg-red-50 p-3 text-xs text-red-700">{error}</div>}
 </div></Modal><MaintenanceScheduleModal open={maintenanceModalOpen} targetHarvestHst={Number(target)||90} value={maintenance} onClose={()=>setMaintenanceModalOpen(false)} onSave={next=>setMaintenance(next)} /><ConfirmDialog open={confirmDelete} title="Hapus Tanggal Polinasi?" message="HSP akan di-reset. Tindakan ini mengubah data siklus aktif." confirmLabel="Hapus" danger onClose={()=>setConfirmDelete(false)} onConfirm={()=>run(async()=>{await onDeleteTanggalPolinasi();setConfirmDelete(false);},"Gagal menghapus tanggal polinasi")} /><ConfirmDialog open={confirmReset} title="Reset Siklus?" message="Siklus aktif akan dikembalikan ke status awal." confirmLabel="Reset Siklus" danger onClose={()=>setConfirmReset(false)} onConfirm={()=>run(async()=>{if(onResetCycle)await onResetCycle();setConfirmReset(false)},"Gagal mereset siklus")} /></>;
}
