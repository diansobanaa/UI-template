...
- **Completed Work**:
  1. Flow Meter Semantic Mapping:
     - Model `ZJ-B1`: Dedicated Raw Water Flow Meter (1â€“25 L/min, $\le$ 1.75 MPa). Process: Raw Water $\to$ Mixing Tank. Pin: GPIO 15. Pulse constant marked `UNVERIFIED / CALIBRATION REQUIRED` (default 0.0 pulses/L). Volumetric conversion deferred until field calibration; pulse accumulation is active. Completion criterion: `actualVolumeMl >= targetVolumeMl`.
     - Model `FS400A G1"`: Dedicated Fertigation Flow Meter (1â€“60 L/min, $\le$ 1.75 MPa, DC 5â€“24V). Process: Fertigation distribution and delivery monitoring. Formula: $F = 4.5 \times Q \implies Q = F / 4.5$; volume calculation factor $270.0\text{ pulses/L}$ ($0.27\text{ pulses/mL}$). Pin: GPIO 16.
...
- **ID**: SP-HW-006
- **Objective**: Integrate native I2C DS3231 RTC HAL driver (GPIO 8 SDA, GPIO 9 SCL), retire legacy DS1302 3-wire bitbang driver, liberate GPIO 47, incorporate physical MOSFET driver pin markings (`TRIG-PWM`, `GND`) into hardware documentation contract, and establish 100% firmware â†” documentation consistency across all 25 pins.
- **Completed Work**:
  ...
  8. Verified 100% firmware â†” documentation match: 25 out of 25 pins match identically; 0 mismatches or unaligned drivers remain.
...
- [x] SP-009 Existing UI â†” ESP32 integration
- [x] SP-010 End-to-end verification
- [x] SP-011 Assembly/commissioning documentation
- [x] SP-AUDIT-001 UI â†” ESP32 Blindspot Audit Complete
- [x] SP-AUDIT-002 UI â†” ESP32 Blindspot Audit Verification Complete
...
- **Completed Work**:
  ...
  2. Created modular `docs/ESP32_GPIO_PIN_MAP.md` covering complete GPIO 0â€“48, header layouts, and reserved/forbidden pins.
...
- **Verification Result**:
  ...
  - Flash: PASS (COM3 @ 460800 baud, hash verified).
  - Boot Test: **PASS â€” SYSTEM READY** (Timestamp 1639 ms, zero WDT resets, zero panics).
...
- **ID**: SP-AUDIT-002
- **Objective**: Complete forensic verification of the 30 identified blindspots in the UI â†” ESP32 codebase.
- **Completed Work**:
...
- **Known Issues / Next Blocker**:
  - `main.c:7:10: fatal error: esp_flash.h: No such file or directory` â€” `main/CMakeLists.txt` missing component requirement `spi_flash`.
- **Next Safe Point / Action**:
...
- **Completed Work**:
  ...
  3. Created comprehensive 17-section readiness deliverable `docs/AI_HARDWARE_COMMISSIONING_READINESS_V1.md` documenting:
     ...
     - UI â†” ESP32 live integration test and failure/recovery test.
...
- **Completed Work**:
  1. Reconciled physical hardware inventory authority against actual ready workbench hardware:
     ...
     - **Display**: Firmly established LCD TFT SPI 1.8 inch (driver ST7735, resolution 128Ã—160 SPI). Prohibited 2.4", 2.8", and ST7789/ILI9341 controllers. Updated `esp32/main/config/pin_config.h` with dedicated definitions (`TFT_DRIVER_ST7735`, `TFT_WIDTH_PX 128`, `TFT_HEIGHT_PX 160`).
