...
> [!NOTE]
> **HISTORICAL DOCUMENTATION (SUPERSEDED):** Flow meter specifications in this historical report reflect early assumptions. For authoritative active specifications, refer to [HARDWARE_INVENTORY.md](file:///d:/template/docs/HARDWARE_INVENTORY.md): **ZJ-B1** for Raw Water Flow on GPIO 15 (calibration required) and **FS400A G1"** for Fertigation Flow on GPIO 16 (288 pulses/L). YF-B1 is obsolete.
...
| **GPIO 40** | `PIN_OUT_MIXING_PUMP`| Output| 220V AC Mixing Pump (Relay IN4) | Aman. Active-LOW control. |
| **GPIO 41** | `PIN_BTN_RESERVED`    | Input | Tombol Panel: RESERVED / UNASSIGNED | Aman. Internal pull-up aktif (Active-LOW: Tekan = 0). Tidak ada fungsi distribusi/operasional. |
