...
| **RELAY** | 4-Channel Relay Board | 4-Channel 5V Optocoupled (VCC-JDVCC jumper installed) | 1 | 5V Coil, 5V Logic | Active-LOW (IN1: GPIO 4, IN2: GPIO 18, IN3: GPIO 10 Booked, IN4: GPIO 40 Mixing Pump) | **VERIFIED READY** | Intermediate galvanic isolation and load switching. IN3 booked for Dual Blower Contactor Trigger. |
...
| **TERM** | AC Terminal Block | TB-1506L (6-Position, 15A 600V) | 1 | 220V AC Mains | Screw Terminals (1-6) | **VERIFIED READY** | Primary 220V AC Mains power distribution (L, N, PE jumpered). |
| **FLOW_RAW**| Raw Water Flow Meter | ZJ-B1 (1–25 L/min, ≤1.75 MPa) Hall Effect | 1 | 5V Power, 3.3V Pulse Out | Pulse Output (GPIO 15 via divider) | **UNVERIFIED (CALIBRATION REQUIRED)** | Inline volumetric metering on raw water transfer line (Raw Water → Mixing Tank). Flow completion criterion. Pulse constant unverified: calibration required. |
| **FLOW_FERT**| Fertigation Flow Meter | FS400A G1" (1–60 L/min, ≤1.75 MPa) Hall Effect | 1 | 5V Power, 3.3V Pulse Out | Pulse Output (GPIO 16 via divider). Red=5V, Yellow=Signal, Black=GND | **VERIFIED READY** | Inline volumetric flow metering on fertigation delivery line. F=4.8×Q (288 pulses/L). Monitoring, telemetry & delivery verification. |
...
| **BTNS** | Momentary Push Buttons | 12mm Tactile Panel Buttons | 4 | 3.3V Dry Contact | Digital In (GPIO 0, 39, 41) | **VERIFIED READY** | Panel buttons: Button 1 (GPIO 0: TFT screen switch), Button 2 (GPIO 39: Well Pump manual toggle with 5-min auto-off & dry-run interlock), Button 3 is RETIRED/disconnected because GPIO 40 is Mixing Pump Relay IN4, Button 4 (GPIO 41) is Reserved/TBD. |
...
| **BLOWER**| Dual Greenhouse Blower Fans | AC 220V High-Volume Exhaust Blowers | 2 | 220V AC Mains | Switched via External Magnetic Contactor / Omron triggered by Relay IN3 (GPIO 10) | **DEFERRED (BOOKED / PLANNED)** | Greenhouse climate and humidity extraction. Deferred until physical investment. |
| **ETH** | W5500 SPI Ethernet Module | W5500 Hardwired TCP/IP | 1 | 3.3V DC Supply | Not mapped in canonical hardware contract | **NOT USED IN CURRENT COMMISSIONING** | Wired LAN interface. GPIO 10 reassigned to Blower Fan Contactor Trigger. |
...
| **FLOW_OLD**| Water Flow Sensor (Old) | YF-B1 Hall Effect | 0 | N/A | Pulse In | **OBSOLETE / REPLACED** | Replaced by ZJ-B1 for raw water flow measurement. |
