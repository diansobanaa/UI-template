...
- **CRITICAL:** UI allows the creation of "Temperature-based" fan schedules. The firmware `api_schedule_handlers.c` parses this, but `scheduler.c` completely ignores it. This gives the operator the false impression that climate control is automated.
...
- **Safety Timeout:** `fertigation_mgr_task()` elapsed time vs flow pulses check.
